...
{n:"ConvLayerSpec", k:"dataclass", s:"out_channels, kernel_size, stride, padding, dilation, pool, tau, v_threshold", o:"Geometry of one convolution, with optional per-layer neuron parameters.",
 b:"tau and v_threshold default to None, meaning the global NeuronSpec applies. The hardware already stores neuron parameters individually, so layers can use different settings. Per-channel parameters were rejected as too many degrees of freedom for this dataset."},
{n:"EncoderSpec", k:"dataclass", s:"depth, channels, kernel_size, stride, layers_json, bias, norm, tdbn_alpha", o:"The convolution stack, given either as uniform fields or as a per-layer JSON list.",
 b:"norm selects none, bn or tdbn. The model enables convolution bias when norm is none. See <a href='architecture.html#perlayer'>3.6</a>."},
...
{n:"TrainSpec", k:"dataclass", s:"epochs, optimizer, lr, weight_decay, scheduler, warmup_epochs, label_smoothing, grad_clip, qat_mode, qat_warmup_frac, qat_epochs, qat_lr_scale, fold_bias_mode, fold_bias_margin", o:"Training settings, including optimizer, learning-rate schedule, and quantization mode.",
 b:"qat_mode selects inline, tail or ptq. fold_bias_mode='threshold' is the only mode the current converter deploys. fold_bias_margin sets how much room the folded threshold keeps above zero, and 0 disables the constraint. See <a href='hardware.html#qat'>4.5</a>."},
...
{n:"HW_TAU_CHOICES", k:"constant", s:"[2, 3, 4, 6, 8, 16, 32, 63]", o:"The integer leak values the search samples.",
 b:"Spaced closely at the low end, where consecutive integers behave differently, and sparsely at the high end, where they do not. 63 was used in a recorded conversion. See <a href='neurons.html#leak'>2.3</a>."},
{n:"_ste", k:"function", s:"_ste(quantized, original) = original + (quantized - original).detach()", o:"Straight-through estimator: the quantized value forward, the gradient to the float parameter backward.",
 b:"Used for weight quantization, threshold quantization, the learned integer leak and the folded-bias constraint. See <a href='hardware.html#ste'>4.4</a>."},
{n:"fold_bias_band / constrain_fold_bias", k:"function", s:"fold_bias_band(v_th, margin) -> (lo, hi) · constrain_fold_bias(b, v_th, margin) -> tensor", o:"Keeps the folded bias inside the interval where theta' = theta - b' is a storable threshold.",
 b:"Both bounds matter: nonpositive thresholds are rejected, and values above w_alpha saturate at INT16_MAX. Applied during inline QAT with a straight-through gradient, so the network trains against the constraint. Measured runs reached b' of about 1.59 against a theta capped at 1.0. See <a href='hardware.html#folding'>4.3</a>."},
{n:"ConvBNFoldQuant.fold_bias_violation", k:"method", s:"-> {frac_out_of_band, min_b, max_b, band}", o:"How much of the unconstrained bias the clamp is currently absorbing, per block.",
 b:"Reports how many unconstrained bias values exceed the threshold-compatible range. Frequent boundary contact indicates that the constraint affects the layer."},
{n:"_no_grad", k:"function", s:"@_no_grad", o:"A no-grad decorator that survives import without torch.",
 b:"DVSGesturePuru is defined at module level, so a bare @torch.no_grad() on one of its methods was evaluated at import and raised when torch was absent. That broke summary mode, which the module docstring promises runs without torch."},
{n:"fake_quantize_weight", k:"function", s:"fake_quantize_weight(w) -> tensor", o:"Reproduces the converter's weight quantization in a differentiable form: clamp to [-1, 1], round to 32,767 steps per side.",
 b:"Clamping during the forward pass lets training respond to the values that will be exported. It does not guarantee a particular change in loss."},
...
{n:"weight_clip_fraction", k:"function", s:"weight_clip_fraction(net) -> float", o:"Proportion of convolution and linear weights outside the representable range.",
 b:"To estimate clipping, inspect the folded weights before clamping. Reading an already clamped tensor makes this fraction zero. The warning threshold in DEPLOY_LIMITS is 2%."},
...
{n:"HardwareLIFNode", k:"class", s:"HardwareLIFNode(tau=2, v_threshold=1.0, v_reset=0.0, learn_tau, learn_threshold, ...)", o:"The neuron this project trains and deploys, matching the chip in update order, integer leak, quantized threshold and undecayed input.",
 b:"hw_tau rounds a learned leak with a straight-through gradient. v_threshold is parametrized through a sigmoid when learned. raw_v_threshold() exposes the unclamped value so the deployment check cannot be satisfied by a safety clamp. Covered throughout <a href='neurons.html'>page 2</a>."},
{n:"_WeightFakeQuant", k:"class", s:"parametrization on module.weight", o:"Makes every read of a weight return its quantized value while gradients reach the float parameter.",
 b:"Registered on every convolution and linear layer by enable_weight_fake_quant."},
{n:"ConvBNFoldQuant", k:"class", s:"ConvBNFoldQuant(conv, bn, bias_mode='threshold', quantize=True)", o:"Replaces a convolution and batch normalization pair with one module that folds and quantizes on every forward pass.",
...
{n:"_LegacyLearnableThresholdLIF", k:"class", s:"retained for older checkpoints", o:"The previous trainable-threshold neuron, which is not deployable.",
 b:"Its continuous tau rounds at conversion and its update order does not match the chip. Its docstring records the reasoning behind learnable thresholds: gradient reaches the threshold only through the surrogate, positivity needs a parametrization rather than a clamp, and weight scale and threshold are jointly under-determined without normalization."},
...
{n:"fold_bias_report", k:"function", s:"fold_bias_report(net) -> [{conv_index, |b'|/theta, theta_prime_min, theta_prime_max, frac out of band}]", o:"Reports how large the folded bias is relative to the threshold, per convolution, and how much of it falls outside the storable band.",
 b:"The ratio is a diagnostic, not a guarantee of equivalent dynamics. Accumulation over time also depends on the leak constant. The two fractions count channels whose theta' would land at or below 0, and above w_alpha. Call before folding, on the evaluation-mode network."},
...
{n:"DEPLOY_LIMITS", k:"constant", s:"min_threshold=0.0 (blocking) · max_weight_clip_frac=0.02 (warning)", o:"The two conditions the deployment audit checks.",
 b:"A folded threshold at or below zero violates the supported range and causes deployment rejection. Heavy clipping is reported but allowed, since quantization-aware training has already had the opportunity to work around it."},
...
{n:"deployment_report", k:"function", s:"deployment_report(net) -> {neurons, weight_clip_frac, min_threshold, deployable, blocking_reasons, warnings}", o:"Audits the folded and quantized network for legality and reports per-layer integer parameters.",
...
{n:"DVSResizeAndBinarize", k:"class", s:"(T, C, H, W) frames -> resized, binary", o:"Interpolates each frame and thresholds at zero.",
 b:"Positive interpolated values become spikes. Resizing changes spatial information and can change spike density. See <a href='training.html#binarize'>5.2</a>."},
...
{n:"build_optimizer", k:"function", s:"build_optimizer(net, train_cfg) -> Adam | AdamW | SGD", o:"Constructs the optimizer, noting that Adam adds weight decay to the gradient while AdamW applies it separately.",
...
{n:"hardware_flush_steps", k:"function", s:"hardware_flush_steps(net) -> int", o:"The number of zero-input timesteps needed to drain the network, equal to the number of spiking layers.",
 b:"Without flush steps, some responses to the final frames may not reach the classifier before evaluation stops. See <a href='training.html#flush'>5.4</a>."},
...
{n:"SEED_CONFIGS", k:"constant", s:"three known configurations evaluated first", o:"Starting points for the sampler, taken from an earlier study.",
 b:"The configurations provide initial candidates. Their earlier floating-point scores are not directly comparable to the converted objective."},
...
{n:"_make_streaming_callback", k:"function", s:"driver-side callback bound to a ResultsWriter", o:"The single process that writes shared files, recording progress, new best results, leaderboard rows and rejections.",
 b:"A pruned trial is recorded as pruned rather than as zero, which would be indistinguishable from a configuration that converted badly. Logging exceptions are caught to allow the search to continue."},
{n:"run_search", k:"function", s:"run_search(args)", o:"Coordinates dataset checks, trial execution, and result export.",
