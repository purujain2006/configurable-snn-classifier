...
{n:"EncoderSpec", k:"dataclass", s:"depth, channels, kernel_size, stride, layers_json, bias, norm, tdbn_alpha", o:"The convolution stack, given either as uniform fields or as a per-layer JSON list.",
 b:"norm selects none, bn or tdbn. The model enables bias when norm is none, since a convolution with neither normalization nor bias holds every output channel at zero mean. See <a href='architecture.html#perlayer'>3.6</a>."},
...
{n:"TrainSpec", k:"dataclass", s:"epochs, optimizer, lr, weight_decay, scheduler, warmup_epochs, label_smoothing, grad_clip, qat_mode, qat_warmup_frac, qat_epochs, qat_lr_scale, fold_bias_mode, fold_bias_margin", o:"The optimization recipe, covering everything that affects training but not shape.",
 b:"qat_mode selects inline, tail or ptq. fold_bias_mode='threshold' is the only route the current converter deploys. fold_bias_margin sets how much room the folded threshold keeps above zero, and 0 disables the constraint. See <a href='hardware.html#qat'>4.5</a>."},
...
{n:"fold_bias_band / constrain_fold_bias", k:"function", s:"fold_bias_band(v_th, margin) -> (lo, hi) · constrain_fold_bias(b, v_th, margin) -> tensor", o:"Keeps the folded bias inside the interval where theta' = theta - b' is a storable threshold.",
 b:"Two-sided, because both ends fail without raising: theta' at or below 0 makes the neuron fire on every timestep, and theta' above w_alpha saturates at INT16_MAX so the deployed threshold is not the trained one. Applied during inline QAT with a straight-through gradient, so the network trains against the constraint. Measured runs reached b' of about 1.59 against a theta capped at 1.0. See <a href='hardware.html#folding'>4.3</a>."},
{n:"ConvBNFoldQuant.fold_bias_violation", k:"method", s:"-> {frac_out_of_band, min_b, max_b, band}", o:"How much of the unconstrained bias the clamp is currently absorbing, per block.",
 b:"Reports pressure rather than damage. A block sitting hard against the band is a sign the layer wants a bias the threshold cannot express, which is the case for fold_bias_mode='conv'."},
...
{n:"fold_bias_report", k:"function", s:"fold_bias_report(net) -> [{conv_index, |b'|/theta, theta_prime_min, theta_prime_max, frac out of band}]", o:"Reports how large the folded bias is relative to the threshold, per convolution, and how much of it falls outside the storable band.",
 b:"A ratio near 0.1 folds into the threshold cleanly. Near 0.5 it does not. The two fractions count channels whose theta' would land at or below 0, and above w_alpha. Call before folding, on the evaluation-mode network."},
